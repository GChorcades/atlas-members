...
const APP_NAME = process.env.BREVO_SENDER_NAME ?? 'Atlas Members';
const APP_URL = process.env.NEXTAUTH_URL ?? 'https://claude-members.vercel.app';
    ...
    <tr><td align="center">
      <table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="max-width:560px;background:#fff;border:1px solid #e5e5e0;border-radius:12px;overflow:hidden;">
        <tr><td style="padding:28px 32px 8px;">
          <div style="font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#888;">${APP_NAME}</div>
...
export async function notifyWelcome(user: User, tempPassword?: string) {
  const title = `Bem-vindo(a) à ${APP_NAME}`;
  const passwordBlock = tempPassword
    ? `<p>Sua senha temporária é:</p>
       ...
       <p>Recomendamos trocar a senha após o primeiro login em <a href="${APP_URL}/profile">${APP_URL}/profile</a>.</p>`
  ...
  const html = baseLayout(
    ...
    `<p>Olá, <strong>${user.name}</strong>!</p>
     ...
     <p style="margin-top:24px;">
       <a href="${APP_URL}/login" style="display:inline-block;background:#1a1a1a;color:#fff;padding:11px 20px;border-radius:8px;text-decoration:none;font-weight:600;">Acessar plataforma</a>
     </p>`,
  );

  const wa = tempPassword
    ? `Olá, ${user.name}! Sua conta na ${APP_NAME} foi criada.\n\n🔐 Senha temporária: ${tempPassword}\n\nAcesse: ${APP_URL}/login\n\nRecomendamos trocar a senha no primeiro acesso.`
    : `Olá, ${user.name}! Sua conta na ${APP_NAME} foi criada.\n\nAcesse: ${APP_URL}/login`;

  return dispatch(user, title, html, wa);
}
...
export async function notifyPasswordReset(user: User, newPassword: string) {
  ...
  const html = baseLayout(
    ...
    `<p>Olá, <strong>${user.name}</strong>.</p>
     ...
     <p>Por segurança, recomendamos alterar a senha após o login em <a href="${APP_URL}/profile">${APP_URL}/profile</a>.</p>
     <p style="margin-top:24px;">
       <a href="${APP_URL}/login" style="display:inline-block;background:#1a1a1a;color:#fff;padding:11px 20px;border-radius:8px;text-decoration:none;font-weight:600;">Fazer login</a>
     </p>`,
  );
  const wa = `Olá, ${user.name}. Sua senha na ${APP_NAME} foi redefinida.\n\n🔐 Nova senha: ${newPassword}\n\nAcesse: ${APP_URL}/login\n\nRecomendamos trocar a senha após o login.`;
  return dispatch(user, title, html, wa);
}
...
export async function notifyForgotPassword(user: User, resetLink: string) {
  ...
  const wa = `Olá, ${user.name}. Pedido de redefinição de senha na ${APP_NAME}.\n\nLink (expira em 1h): ${resetLink}\n\nSe não foi você, ignore esta mensagem.`;
  return dispatch(user, title, html, wa);
}

export async function notifyPaymentConfirmed(user: User, args: { description: string; value: number }) {
  ...
  const html = baseLayout(
    ...
    `<p>Olá, <strong>${user.name}</strong>!</p>
     ...
     <p style="margin-top:24px;">
       <a href="${APP_URL}/dashboard" style="display:inline-block;background:#1a1a1a;color:#fff;padding:11px 20px;border-radius:8px;text-decoration:none;font-weight:600;">Ir para o painel</a>
     </p>`,
  );
  const wa = `✅ Pagamento confirmado!\n\nOlá, ${user.name}. Recebemos seu pagamento de ${valueFmt} (${args.description}).\n\nSeu acesso está liberado: ${APP_URL}/dashboard`;
  return dispatch(user, title, html, wa);
}

async function dispatch(user: User, subject: string, htmlContent: string, waMessage: string) {
  const tasks: Promise<unknown>[] = [];
  tasks.push(
    sendEmail({ to: { email: user.email, name: user.name }, subject, htmlContent }).catch((e) => {

...
export async function GET(_req: Request, { params }: { params: Promise<{ lessonId: string }> }) {
  ...
  const [course] = mod
    ? await db.select({ title: courses.title }).from(courses).where(eq(courses.id, mod.courseId)).limit(1)
    : [null];

  const [pandaPlayer] = await db.select({ value: settings.value }).from(settings).where(eq(settings.key, 'panda_player_id')).limit(1);
  const [pandaKey] = await db.select({ value: settings.value }).from(settings).where(eq(settings.key, 'panda_api_key')).limit(1);
